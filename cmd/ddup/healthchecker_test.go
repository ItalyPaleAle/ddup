...
func TestHealthChecker_AllHealthy(t *testing.T) {
	...
	// Create mock checker
	mockChecker := &healthcheck.MockChecker{
		Domain:      "example.com",
		MaxAttempts: 2,
		Results:     results,
	}
	...
	actualIPs := hc.domainCheckers[0].healthyIPs

	assert.ElementsMatch(t, expectedIPs, actualIPs, "Healthy IPs should match expected")
	assert.Empty(t, hc.domainCheckers[0].failedIPs, "Expected no failed IPs")
...
func TestHealthChecker_SomeUnhealthy(t *testing.T) {
	...
	assert.ElementsMatch(t, expectedIPs, actualIPs, "Healthy IPs should match expected")
	assert.Len(t, hc.domainCheckers[0].failedIPs, 1, "Expected 1 failed IP")
	assert.Equal(t, 1, hc.domainCheckers[0].failedIPs["2.2.2.2"], "Expected failed IP 2.2.2.2 to have 1 attempt")
...
func TestHealthChecker_RetryLogic(t *testing.T) {
	...
	hc.checkAndUpdateDNS(t.Context())
	assert.Len(t, hc.domainCheckers[0].healthyIPs, 2, "After second failure: Expected 2 healthy IPs")
	assert.Equal(t, 2, hc.domainCheckers[0].failedIPs["2.2.2.2"], "Expected failed IP 2.2.2.2 to have 2 attempts")
	assert.Equal(t, 0, mockProvider.CallCount)

	// Third check - endpoint2 fails a third time, should now be considered unhealthy
	hc.checkAndUpdateDNS(t.Context())
	assert.Len(t, hc.domainCheckers[0].healthyIPs, 1, "After third failure: Expected 1 healthy IP")
	assert.Equal(t, "1.1.1.1", hc.domainCheckers[0].healthyIPs[0], "Expected remaining healthy IP to be 1.1.1.1")
	assert.Equal(t, 3, hc.domainCheckers[0].failedIPs["2.2.2.2"], "Expected failed IP 2.2.2.2 to have 3 attempts")
	assert.Equal(t, 1, mockProvider.CallCount)
}
...
func TestHealthChecker_AllUnhealthyNoUpdate(t *testing.T) {
	...
	assert.Empty(t, hc.domainCheckers[0].healthyIPs, "Expected 0 healthy IPs when all endpoints are unhealthy")
	assert.Len(t, hc.domainCheckers[0].failedIPs, 2, "Expected 2 failed IPs")
...
func TestHealthChecker_DNSProviderError(t *testing.T) {
	...
	assert.Empty(t, hc.domainCheckers[0].healthyIPs, "Expected healthy IPs to remain unchanged due to DNS error")
}
...
func TestHealthChecker_MultipleDomains(t *testing.T) {
	...
	results2 := []healthcheck.Result{
		{Endpoint: endpoints2[0], Healthy: true},
	}

	// Create mock checkers
	mockChecker1 := &healthcheck.MockChecker{
		Domain:      "example.com",
		MaxAttempts: 2,
		Results:     results1,
	}

	mockChecker2 := &healthcheck.MockChecker{
		Domain:      "test.com",
		MaxAttempts: 2,
		Results:     results2,
	}
	...
	actualIPs1 := hc.domainCheckers[0].healthyIPs

	assert.ElementsMatch(t, expectedIPs1, actualIPs1, "Domain 1: Healthy IPs should match expected")
	assert.Equal(t, 1, hc.domainCheckers[0].failedIPs["2.2.2.2"], "Domain 1: Expected failed IP 2.2.2.2 to have 1 attempt")

	expectedIPs2 := []string{"3.3.3.3"}
	actualIPs2 := hc.domainCheckers[1].healthyIPs
	assert.ElementsMatch(t, expectedIPs2, actualIPs2, "Domain 2: Healthy IPs should match expected")
	assert.Empty(t, hc.domainCheckers[1].failedIPs, "Domain 2: Expected no failed IPs")
